...
class OpponentSelectForm(forms.Form):
    opp_draft_pick = forms.ModelChoiceField(queryset=Hero.objects.all(), empty_label="Select an Opponent Hero")
    def __init__(self, *args, **kwargs):
...
class AllyBanForm(forms.Form):
    ally_ban = forms.ModelChoiceField(queryset=Hero.objects.all(), empty_label="Select a Hero to Ban")
    def __init__(self, *args, **kwargs):
        super(AllyBanForm, self).__init__(*args, **kwargs)
        self.fields['ally_ban'].label = "Your Ban"

class OpponentBanForm(forms.Form):
    opp_ban = forms.ModelChoiceField(queryset=Hero.objects.all(), empty_label="Select a Hero to Ban")
    def __init__(self, *args, **kwargs):
